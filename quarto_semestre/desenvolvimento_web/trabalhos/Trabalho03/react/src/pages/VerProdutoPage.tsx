...
function VerProdutoPage() {
  const { id } = useParams();
  const { data, isLoading, isError } = useVerProduto(parseInt(id || ""));

  if (isLoading) return <div className="container mt-5">Carregando...</div>;
  if (isError)
    return (
  ...
  if (!data)
    return (
      <div className="container mt-5">Dados do produto não encontrados.</div>
    );
